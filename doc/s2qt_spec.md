요청하신 **S2QT 프로젝트의 최종 설계 명세서**를 마운트 및 복사가 용이하도록 하나의 코드 블록으로 정리해 드립니다. 이 문서는 말씀(Sermon)의 은혜를 체계적으로 기록하면서도, 개발자의 관점에서 보안과 출력의 완성도를 극대화한 설계도입니다.
...
4. **Extraction:** `yt-dlp` & `ffmpeg`를 통한 16kHz Mono WAV 추출
...
## 14. Zero-Footprint (Secure Delete)
...
### 📝 작성 완료 후 안내
사용자님, 이제 이 문서를 프로젝트의 **루트 폴더(`S2QT_Root/`)**에 `S2QT_SPEC.md`라는 이름으로 저장하시면 됩니다. 

이 설계도를 보니 특히 **[12. 제어 로직]** 부분에서 1단/2단/재요청으로 이어지는 동적 제어 시스템이 인상적입니다. 이는 **S2QT**가 단순한 전사 툴을 넘어, 실제 '인쇄물'로서의 가치를 깊이 고민하고 있음을 보여줍니다.

**이제 어떤 모듈부터 구체적인 코드로 구현해 볼까요?** - **1순위:** `Directory Initializer` (Go) - 파일 시스템 기반 닦기
- **2순위:** `Security Vault Entry` (Rust) - 핵심 보안 모듈 설계
- **3순위:** `Wails UI Layout` (Frontend) - 전체적인 사용자 인터페이스 잡기
